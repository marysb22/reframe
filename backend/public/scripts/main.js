...
// Looks up a translations.js key in the page's current language, for the
// handful of user-facing strings (form validation/success/error messages)
// that are generated by this script rather than sitting in the markup as
// data-translate text. Falls back to the English text passed in if the
// key or the translations object itself isn't available for some reason.
...
function initializeNewsletterForm() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!validateEmail(email)) {
      showMessage(t("msg_invalid_email", "Please enter a valid email address."), "error");
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (successMessage) {
        ...
        showMessage(t("msg_newsletter_success", "Thank you for subscribing! We'll keep you updated."), "success");
      }
    }, 1500);
  });
}
...
function initializeContactForm() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (missingFields.length > 0) {
      showMessage(t("msg_required_fields", "Please fill in all required fields."), "error");
      return;
    }
    ...
    setTimeout(() => {
      ...
      form.reset();
      showMessage(t("msg_contact_success", "Thank you for your message! We'll get back to you soon."), "success");
    }, 2000);
  });
}
...
function initializeFormspreeContact() {
  ...
  contactFormSpree.addEventListener("submit", function (e) {
    ...
    fetch(formspreeURL, {
      ...
      .then((response) => {
        if (response.ok) {
          statusDiv.innerText = t("msg_formspree_success", "Message sent successfully! 😊");
          ...
          response.json().then((data) => {
            if (data.errors) {
              ...
              statusDiv.innerText = t("msg_send_error", "Oops! There was a problem sending your message.");
            }
          });
        }
      })
...
function initializeLabSubscribe() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!isValid(v)) {
      ...
      msg.textContent = t("msg_invalid_email_short", "Please enter a valid email.");
    ...
    try {
      ...
      msg.textContent = t("msg_subscribe_confirm", "Thanks! Check your inbox to confirm.");
      msg.className = "form-message success";
      form.reset();
    } catch {
      wrap.classList.remove("loading");
      msg.textContent = t("msg_generic_error", "Something went wrong. Try again.");
      msg.className = "form-message error";
    } finally {
...
function initializeAccessibility() {
  ...
  skipLink.textContent = t("a11y_skip_link", "Skip to main content");
  // This runs before loadLanguagePreference() applies the saved language
  // (see the DOMContentLoaded handler below), so document.documentElement.lang
  // is still the page's static default at the line above -- tagging the
  // element itself means the subsequent full-page switchLanguage() pass
  // (which walks every [data-translate] element) picks it up and corrects
  // it to the actual saved language, same as any other translated element.
  skipLink.setAttribute("data-translate", "a11y_skip_link");
...
function initializeCalendar() {
  ...
  eventDays.forEach((day) => {
    day.addEventListener("click", function () {
      showMessage(t("msg_event_details_placeholder", "Event details would appear here in a full implementation."), "info");
    });
  });
}
...
function initializeVideoPlayers() {
  ...
  playButtons.forEach((button) => {
    button.addEventListener("click", function () {
      showMessage(t("msg_video_placeholder", "Video player would open here in a full implementation."), "info");
    });
  });
}
...
function showLoading(button, isLoading) {
  ...
  if (isLoading) {
    ...
    button.textContent = t("msg_loading", "Loading...");
