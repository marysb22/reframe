...
/** Confirms studentId is currently assigned to the calling ToT and returns their row, or writes a 403/404 and returns null.
 *  (Param/variable names kept as student/supervisor throughout this file -- they match the actual DB tables and columns;
 *  only the JSON error strings returned to the client, which the dashboard surfaces via showToast, are renamed below.) */
async function loadAssignedStudent(db, supervisorId, studentId, res) {
    const { rows: assignRows } = await db.query(
        "SELECT 1 FROM supervisor_students WHERE supervisor_id = $1 AND student_id = $2", [supervisorId, studentId]
    );
...
// ---- Trainees (ToT's own caseload -- route paths/table/column names
// unchanged: /students, supervisor_students, student_id, etc.) ----------
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       ORDER BY st.full_name`, [req.user.id]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       WHERE uc.member_code = $1`, [String(studentCode).trim().toUpperCase()]
        );
        ...
        await db.query(
            "INSERT INTO supervisor_students (supervisor_id, student_id, assigned_by) VALUES ($1, $2, $1) ON CONFLICT DO NOTHING", [req.user.id, student.id]
        );
        await db.query(
            "INSERT INTO audit_logs (actor_id, action, entity_type, entity_id) VALUES ($1, 'supervisor_assigned', 'supervisor_students', $2)", [req.user.id, student.id]
        );
...
router.delete(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        await db.query(
            "INSERT INTO audit_logs (actor_id, action, entity_type, entity_id) VALUES ($1, 'supervisor_unassigned', 'supervisor_students', $2)", [req.user.id, studentId]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        switch (recordType) {
           ...
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id`, [studentId, req.user.id, sessionType, title || null, date || null, time || null, durationMinutes || null, content || null]
                    );
           ...
           VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [studentId, req.user.id, date || null, status, content || null, req.user.id]
                    );
                    insertedId = rows[0].id;
                    break;
                }
            ...
            case "supervision_hours":
                {
                    ...
                    const { rows } = await db.query(
                        `INSERT INTO ${table} (student_id, supervisor_id, hours, hour_date, description) VALUES ($1,$2,$3,$4,$5) RETURNING id`, [studentId, req.user.id, hours, date || null, content || null]
                    );
                    insertedId = rows[0].id;
                    break;
                }
           ...
           VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [studentId, req.user.id, title || "Untitled assignment", content || null, date || null, status || "pending"]
                    );
                    insertedId = rows[0].id;
                    break;
                }
            case "note":
                {
                    const { rows } = await db.query(
                        `INSERT INTO supervisor_notes (student_id, supervisor_id, note_date, content) VALUES ($1,$2,$3,$4) RETURNING id`, [studentId, req.user.id, date || null, content || ""]
                    );
                    insertedId = rows[0].id;
                    break;
                }
           ...
           VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`, [studentId, req.user.id, date || null, title || null, score ?? null, content || null]
                    );
                    insertedId = rows[0].id;
                    break;
                }
...
router.put(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        const { rows: assignRows } = await db.query(
            "SELECT 1 FROM supervisor_students WHERE supervisor_id = $1 AND student_id = $2", [req.user.id, existing.student_id]
        );
        ...
        if (recordType === "training_session" || recordType === "supervision_session") {
         ...
         WHERE id = $6`, [date ?? null, time ?? null, durationMinutes ?? null, title ?? null, content ?? null, recordId]
            );
        } else if (recordType === "attendance") {
            await db.query(
                `UPDATE attendance SET attendance_date = COALESCE($1, attendance_date), status = COALESCE($2, status), notes = COALESCE($3, notes) WHERE id = $4`, [date ?? null, status ?? null, content ?? null, recordId]
            );
            ...
            await db.query(
                `UPDATE ${meta.table} SET hour_date = COALESCE($1, hour_date), hours = COALESCE($2, hours), description = COALESCE($3, description) WHERE id = $4`, [date ?? null, hours, content ?? null, recordId]
            );
        } else if (recordType === "assignment") {
            await db.query(
                `UPDATE assignments SET due_date = COALESCE($1, due_date), status = COALESCE($2, status), title = COALESCE($3, title), description = COALESCE($4, description), updated_at = now() WHERE id = $5`, [date ?? null, status ?? null, title ?? null, content ?? null, recordId]
            );
        } else if (recordType === "note") {
            await db.query(
                `UPDATE supervisor_notes SET note_date = COALESCE($1, note_date), content = COALESCE($2, content), updated_at = now() WHERE id = $3`, [date ?? null, content ?? null, recordId]
            );
        } else if (recordType === "evaluation") {
            await db.query(
                `UPDATE evaluations SET evaluation_date = COALESCE($1, evaluation_date), title = COALESCE($2, title), score = COALESCE($3, score), content = COALESCE($4, content), updated_at = now() WHERE id = $5`, [date ?? null, title ?? null, score ?? null, content ?? null, recordId]
            );
        }
...
router.delete(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        const { rows: assignRows } = await db.query(
            "SELECT 1 FROM supervisor_students WHERE supervisor_id = $1 AND student_id = $2", [req.user.id, existingRows[0].student_id]
        );
        if (!assignRows.length) return res.status(403).json({ error: "You are not assigned to this trainee" });
...
router.post("/students/:studentId/documents", (req, res) => {
    documentUpload.single("document")(req, res, async(err) => {
        ...
        const assigned = await pool.query(
            "SELECT 1 FROM supervisor_students WHERE supervisor_id = $1 AND student_id = $2", [req.user.id, studentId]
        );
       ...
       RETURNING *`, [studentId, req.user.id, req.file.filename, req.file.originalname]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        const chatId = await getOrCreateChat(db, req.user.id, studentId);
        const { rows } = await db.query(
            "INSERT INTO messages (chat_id, sender_id, content) VALUES ($1, $2, $3) RETURNING *", [chatId, req.user.id, content.trim()]
        );
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
        const { rows } = await db.query(
            "SELECT * FROM learning_materials WHERE supervisor_id = $1 ORDER BY created_at DESC", [req.user.id]
        );
...
router.post("/materials", (req, res) => {
    ...
    if (contentType.includes("multipart/form-data")) {
        materialUpload.single("file")(req, res, async(err) => {
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [req.user.id, studentId || null, title, description || null, materialType, req.file.filename, req.file.originalname]
            );
    ...
    (async() => {
       ...
       VALUES ($1,$2,$3,$4,'link',$5) RETURNING *`, [req.user.id, studentId || null, title, description || null, externalUrl]
        );
        res.status(201).json(toMaterial({...rows[0], supervisor_name: req.user.member_code }));
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
        const { rows } = await db.query(
            "SELECT * FROM announcements WHERE supervisor_id = $1 ORDER BY created_at DESC", [req.user.id]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        const { rows } = await db.query(
            "INSERT INTO announcements (supervisor_id, title, content) VALUES ($1, $2, $3) RETURNING *", [req.user.id, title, content]
        );
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       ORDER BY s.session_date ASC, s.session_time ASC NULLS LAST`, [req.user.id]
        );
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       ORDER BY al.created_at DESC LIMIT 15`, [req.user.id]
        );
        res.json({
...
router.get(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       ORDER BY m.scheduled_at ASC NULLS LAST`, [req.user.id]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [req.user.id, studentId || null, title, platform, meetingUrl, scheduledAt || null, durationMinutes || null]
        );
...
router.post(
    ...
    asyncRoute(async(req, res, db) => {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`, [req.user.id, studentId || null, type, title, description || null, date, time || null]
        );
...
router.put(
    ...
    asyncRoute(async(req, res, db) => {
        ...
        const { rows: existingRows } = await db.query(
            "SELECT * FROM calendar_events WHERE id = $1 AND owner_id = $2", [eventId, req.user.id]
        );
       ...
       WHERE id = $5 RETURNING *`, [title ?? null, description ?? null, date ?? null, time ?? null, eventId]
        );
